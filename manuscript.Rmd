...
```{r tblJusticeTypes}
knitr::kable(tblJusticeTypes %>% set_names(c("Cluster","Type","Total")), caption= cap, 
             booktabs = T)
```
...
tableLDAResultsShape = rio::import(here('data/LDAResultsShape.xlsx')) %>% 
  mutate(Mean = case_when(Shape == "Mean"~NA,TRUE~Mean))
...
tableLDAResultsType = rio::import(here('data/LDAResultsType.xlsx')) %>% 
  mutate(Mean = case_when(Type == "Mean"~NA,TRUE~Mean))
...
tableLDAResultsCluster = rio::import(here('data/LDAResultsCluster.xlsx')) %>% 
  mutate(Mean = case_when(Cluster == "Mean"~NA,TRUE~Mean))
...
```{r JusticeEFAPCA, fig.cap= 'Principal components (PC) plot showing projectile points from Justice (2002) and the morphospace based on an elliptical Fourier analysis. The projectile points are labeled by the cluster assigned by Justice. The barchart in bottom right corner displays proportion each PC contributes to total variance.'}
figs = figs + 1
...
All relevant data and scripts are available at the following [DOI 10.17605/OSF.IO/ZGE9Q](https://osf.io/zge9q/) and on GitHub at \url{https://github.com/bischrob/TontoBasinPoints}. The R code used in the analysis is included in the manuscript.Rmd file used to create this manuscript, although some lines have been commented out to improve efficiency.
